...
function my_custom_checkout_field( $checkout ) {	
	...
	jQuery("#e_deliverydate").parent().after("<div id=\'order-desc\'><small style=font-size:10px;>Wij zullen ons best doen om de bestelling te leveren op de gekozen datum</small></div>");
});</script>';
	...
	woocommerce_form_field( 'e_deliverydate', array(        
				...
				'label'         => __('Leveringsdatum'),		

				'required'  	=> true,		

				'placeholder'       => __('Leveringsdatum'),        
...
function my_custom_checkout_field_update_order_meta( $order_id ) {    
	...
	if ($_POST['e_deliverydate']) {
		...
		update_post_meta( $order_id, 'Leveringsdatum', esc_attr($_POST['e_deliverydate']));
